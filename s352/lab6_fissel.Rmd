...
```{r}
gamma_fun = function(rate_0,z,fit_HA,cutoff,conf_level){
  ##AJW## the restriction is rate_0 >0
  #if(abs(rate_0)>=1){return(Inf)}
  if(rate_0<=0){return(Inf)}
  if(missing(fit_HA)){
    fit_HA = mle_gamma(z)
  }
  if(missing(cutoff)){
  ...
  fit_H0 = mle_gamma_fixed(z,rate_0)
  lambda = 2*fit_HA$loglik - 2*fit_H0$loglik
  return(lambda-cutoff)
}
```
...
```{r}
conf_level = 0.95
cutoff = qchisq(conf_level,df=2)
fit_HA = mle_gamma(z)
##AJW## I do not know why there is any math here, just access the rate
#rate_hat = fit_HA$theta['shape'] - fit_HA$theta['rate']
rate_hat =  fit_HA$theta['rate']
names(rate_hat) = NULL

lb = uniroot(gamma_fun,c(0,rate_hat),
             z=z,fit_HA = fit_HA, cutoff=cutoff,
             conf_level = conf_level)$root

ub = uniroot(gamma_fun,c(rate_hat,rate_hat+1), extendInt = 'upX',
             z=z,fit_HA = fit_HA, cutoff=cutoff,
             conf_level = conf_level)$root
...
The first interval contains the $\widehat{\delta}$ which is good, it needs too. 
The interval also is quite small, likely means it was highly precise with lots of data or very little data. (The data seems to be pretty large.)
The second interval contains the $\widehat{rate}$ which is good, again it should.
This interval is much bigger here, and the data is still pretty big. Which means there's more variation in the data. Contrast with the first interval.

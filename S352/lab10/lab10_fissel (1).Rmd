...
```{r}
post_ests_phi0_1_n1 = (n_1*mean_y_1+1)/(n_1+1)
post_ests_phi0_10_n1 = (n_1*mean_y_1+1)/(n_1+10)
post_ests_phi0_1_n2 = (n_2*mean_y_2+1)/(n_2+1)
post_ests_phi0_10_n2 = (n_2*mean_y_2+1)/(n_2+10)
mle_n1 = mean_y_1
mle_n2 = mean_y_2

#plots
plot(density(mle_n1),ylim=c(0,3))
lines(density(post_ests_phi0_1_n1),col="red",lty=2)
lines(density(post_ests_phi0_10_n1),col="blue",lty=3)

plot(density(mle_n2),ylim=c(0,6))
lines(density(post_ests_phi0_1_n2),col="red",lty=2)
lines(density(post_ests_phi0_10_n2),col="blue",lty=3)

#risks
mean((mle_n1-theta_star)^2)
mean((post_ests_phi0_1_n1-theta_star)^2)
mean((post_ests_phi0_10_n1-theta_star)^2)

mean((mle_n2-theta_star)^2)
mean((post_ests_phi0_1_n2-theta_star)^2)
mean((post_ests_phi0_10_n2-theta_star)^2)
```
Note that when $\phi_0$ is 1 the posterior estimates the MLE quite closely. When sample size increases, we the data more can more easily overcome the prior distribution. 
However, when $\phi_0$ is 10 the posterior estimate is different from the MLE and has less variance in first graph, but not really in the second.
...
```{r}
log_bayes_fac = function(ybar,n,theta0,phi0){
  n*ybar*log(theta0) - n*theta0 + (n*ybar+1)*log(phi0+n) - lfactorial(n*ybar)-log(phi0)
}

plot(density(log_bayes_fac(mean_y_1,20,1,1)),xlim=c(-3,10))
lines(density(log_bayes_fac(mean_y_1,20,1,10)),col='blue')
plot(density(log_bayes_fac(mean_y_2,200,1,1)),xlim=c(-6,10))
lines(density(log_bayes_fac(mean_y_2,200,1,10)),col='blue')

plot(density(log_bayes_fac(mean_y_1,20,2,1)),xlim=c(-20,15))
lines(density(log_bayes_fac(mean_y_1,20,2,10)),col='blue')
plot(density(log_bayes_fac(mean_y_2,200,2,1)),xlim=c(-100,-20))
lines(density(log_bayes_fac(mean_y_2,200,2,10)),col='blue')

```
The graphs show that when the sample size increases from 20 to 200, there is a large difference in the data's ability to overcome the prior distribution when $\theta_0$ is well chosen. When the $\phi_0$ is not well chosen we see the data struggle to overcome the prior. Both sets of graphs show us the effect on our centers, where our data exists, when our variables are well chosen vs. not-well chosen. 


## 3.3 Task 3:
For $\phi_0$ being $1$ and $10$ and $\theta_0$ being $1$ and $2$, make density plots of $BIC(M_0)-BIC(M_1)$ for each choice of sample size (you should have $8$ densities total). From these results, what can you say about differences in model comparison when $\theta_0$ is well chosen (the same as $\theta^*=1$) and when $\phi_0$ is well chosen ($E[\theta|M_0]=\theta^*=1$) and when they are not? What is the effect of increasing the sample size from $20$ to $200$?
```{r}
BIC_diff = function(n,ybar,theta0){
  2*n*ybar*log(ybar) - 2*n*ybar - log(n) - (2*n*ybar*log(theta0)-2*n*theta0)
}
plot(density(BIC_diff(20,mean_y_1,1)))
plot(density(BIC_diff(20,mean_y_1,2))) 

plot(density(BIC_diff(200,mean_y_2,1)))
plot(density(BIC_diff(200,mean_y_2,2)))
```
When $\theta_0$ is well chosen the data has a more difficult time overcoming the prior in this case. In other words graph 1 gathers evidence quickly to the chosen distribution while graph 2 gathers evidence slower to a different conclusion. While when $\theta_0$ is not well chosen the data more easy overcomes the prior distribution and moves much more. (Differences between graphs 1&3 and 2&4) The sample size gives our data space to adjust the distribution, provided the $\theta_0$ is not well chosen. 
...
n=30
y_bar = 4.5
post_ests_phi0_1_n = (n*y_bar+1)/(n+1)
post_ests_phi0_2_n = (n*y_bar+1)/(n+0.2)

N_samp = 10000
theta_draw = post_ests_phi0_1_n 
...
percents
```
When $\phi_0$ is 1, the percentages are slightly skewed to the left of the mean, while when $\phi_0$ is 0.2, the percentages are slightly skewed past mean. Generally more of the whole distribution is [1,4] in the first set, while more of the whole distribution is [4,10] in the second set. 

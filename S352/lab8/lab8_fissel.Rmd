...
```{r}
stat_fun2 = function(x,ind=1:nrow(x)){
  x = x[ind,]
  stat_out = stat_fun(x)
  ...
  corr_sw_sl = stat_out['covariances.Sepal.Length_Sepal.Width']/ ##AJW## fixed my issue
    sqrt(
    stat_out['variances.Sepal.Width']*
      stat_out['variances.Sepal.Length']
  )
...
intervals = list()
for(i in 1:3){ ##AJW## only loop to 3
  intervals[[i]] = boot.ci(boot_out,type="basic",index=i)
}
intervals
...
```{r}
stat_fun3 = function(x,ind=1:nrow(x)){
  x = x[ind,]
  corr_out = stat_fun2(x)
  d = corr_out[3] - corr_out[2]*corr_out[1]
  #corr_out[1] - corr_out[2]*corr_out[3]
  #corr_out[2] - corr_out[3]*corr_out[1]
  out = c(d)
  return(out)
}
```
```{r}
boot_out = boot(x,stat_fun3,10000)
intervals = boot.ci(boot_out,type="basic")
intervals
names(intervals) = names(stat_fun3(x))
```
...
```{r}
lm_coef_fun = function(x,ind=1:nrow(x)){
x = x[ind,]
mod = lm(x[,3]~x[,1]+x[,2])
...
```
```{r}
boot_out = boot(x,lm_coef_fun,10000)
intervals = boot.ci(boot_out,type='basic')
intervals
names(intervals) = names(lm_coef_fun(x))
```
...
I would be unable to reject the null hypothesis, as the interval contains 0, however it is slightly heavier tailed on the right side of 0, but still does include 0. 
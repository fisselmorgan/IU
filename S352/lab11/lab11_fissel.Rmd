...
sampler_poisson = function(y,N_samps=10000){
  ...
  for(i in 1:N_samps){
    theta_curr = rgamma(1,s_n+1,n+1)
    phi_curr = rgamma(1,2,(1+theta_curr))
...
```{r}
out = sampler_poisson(y)
theta_draws = out[['theta']]
plot(theta_draws,pch=".")
acf(theta_draws)
phi_draws = out[['phi']]
plot(log(phi_draws),pch=".")
acf(phi_draws)
```
Looks relatively reasonable, note though the spread on the trace plot is kind of wide given the distance between the dots. Correlation isn't too great, but isn't that bad.(Theta) 
I would say phi is about as reasonable even, tighter spread on the trace and a similar ACF plot to theta.
...
sampler_negative_binomial = function(y,N_samps=10000){
  ...
  for(i in 1:N_samps){
    theta_curr = rgamma(1,s_n+1,t_n_curr+phi_curr)
    phi_curr = rgamma(1,2,(1+theta_curr))
    z_curr = rgamma(n,y+nu_curr,theta_curr+nu_curr)
    t_n_curr = sum(z_curr)
    u_n_curr = sum(log(z_curr))
    nu_curr = draw_nu_fun(nu_curr,t_n_curr,u_n_curr,n)
...
```{r}
out = sampler_negative_binomial(y)
theta_draws = out[['theta']]
phi_draws = out[['phi']]
nu_draws = out[['nu']]
plot(theta_draws,pch='.')
...
plot(log(phi_draws),pch=".")
acf(phi_draws)
plot(log(nu_draws),pch=".")
acf(nu_draws)
```
theta and phi draws are relatively fine, however... EWWWW nu's draws are baddddd, heavy correlation too.
...
theta_draws = out[['theta']]
phi_draws = out[['phi']]
chib_theta = chib_poisson_using_theta(y,theta_draws,phi_draws)
chib_phi = chib_poisson_using_phi(y,theta_draws,phi_draws)
plot(density(theta_draws))
plot(density(phi_draws))
quantile(theta_draws,c(0.025,0.975))
quantile(phi_draws,c(0.025,0.975))
```
...
phi_draws = out[['phi']]
nu_draws = out[['nu']]
chib_neg = chib_negative_binomial(y,theta_draws,phi_draws,sum(nu_draws),sum(log(nu_draws)),nu_draws)
plot(density(theta_draws))
plot(density(phi_draws))
plot(density(nu_draws))
quantile(theta_draws,c(0.025,0.975))
quantile(phi_draws,c(0.025,0.975))
quantile(nu_draws,c(0.025,0.975))
```
I think something might be incorrect in my negative binomial, but the poisson seems to better describe our data here, based on the chib calculations.
...
n = 30 
theta_draw = rpois(length(theta_draws),theta_draws)
pred_draw_1 = rpois(N_samp,theta_draw) #vectorization being used!
percents1 = rep(NA,11)
names(percents1) = c(0:10)
for(i in 1:11){
  percents1[i] = mean(pred_draw_1==(i-1))
}
percents1
```
bias theta to zero

this is just for a code example)
```{r}
N_samp = 10000

n = 30 
theta_draw = rnbinom(length(theta_draws),size=nu_draws,mu=theta_draws)
pred_draw_2 = rpois(N_samp,theta_draw) #vectorization being used!
percents2 = rep(NA,11)
names(percents2) = c(0:10)
for(i in 1:11){
  percents2[i] = mean(pred_draw_2==(i-1))
}
percents2
```
this is def not biased to zero. 
